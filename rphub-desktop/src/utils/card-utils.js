...
function mapExportItems(items, mapper) {
    return Array.isArray(items) ? items.map((item, index) => mapper(item, index)) : [];
}
...
function toWorldInfoExportEntry(entry = {}) {
    return {
...
function toRegexExportEntry(script = {}) {
...
function toUiTemplateExportEntry(template = {}, options = {}) {
...
function buildCharacterCardData(character = {}, options = {}) {
...
function imageUrlToPngBytes(src, options = {}) {
    return new Promise((resolve, reject) => {
        const img = new Image();
